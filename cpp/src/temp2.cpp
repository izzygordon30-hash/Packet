...
[[nodiscard]] auto from_bytes(std::vector<uint8_t> bytes) noexcept -> std::vector<char> {

    auto name_len = reinterpret_cast<std::size_t>(bytes.data() + (sizeof(std::int32_t)));
    std::vector<char> name_buf;
    for (char i = 0; i < name_len; ++i) {
        name_buf.push_back(*reinterpret_cast<char*>(bytes.data() + 4 + i));
    }
